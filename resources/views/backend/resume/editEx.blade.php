...
@section('content')
    ...
    <div class="card mb-4">
        <div class="card-header">
            <h6 class="fw-bold">Edit Experience</h6>
        </div>
        <form action="{{ route('experience.update', ['id' => $experience->id]) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            <div class="card-body">
                <div class="form-group mb-3">
                    <label>Name</label>
                    <input type="text" name="name" id="name" class="form-control form-control-sm" value="{{ $experience->name }}">
                </div>
                <div class="form-group mb-3">
                    <label>Title</label>
                    <input type="text" name="title" id="title" class="form-control form-control-sm" value="{{ $experience->title }}">
                </div>
                <div class="form-group mb-3">
                    <label for="start">Start</label>
                    <input type="month" name="start" class="form-control" id="start" rows="3" value="{{ $experience->start }}">
                </div>
                <div class="form-group mb-3">
                    <label for="end">End</label>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="end_option" id="ex_now" value="Now">
                        <label class="form-check-label" for="ex_now">
                            Now (Ongoing)
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="end_option" id="ex_date_option" value="Specify Date">
                        <label class="form-check-label" for="ex_date_option">
                            Specify Date
                        </label>
                        <input class="form-control" type="month" name="end_date" id="ex_date_input" value="{{ $experience->end }}">
                    </div>
                </div>
                <div class="form-group mb-3">
                    <label for="exp_description">Description</label>
                    <textarea name="exp_description" class="form-control" id="exp_description" rows="3">{{ $experience->description }}</textarea>
                </div>
